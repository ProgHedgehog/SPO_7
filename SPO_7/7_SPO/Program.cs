...
namespace _7_SPO
{
    class Program
    {
        ...
        {
            string[] input = File.ReadAllText(@"C:\Users\User\Desktop\Универ\СПО\6\6_SPO\6_SPO\bin\Debug\input.txt")
                .Replace("\r", "").Replace("\n", "").Split(' ');
            ...
            for (int i = 0; i < input.Length; i++)
            {
                ...
                //if (input[i] == ":=")
                //{
                //    if ((!Regex.IsMatch(input[i - 1], @"[a-z]"))&&(input[i-2].ToLower()=="then"))
                //    {
                //        Console.WriteLine("Неверная левая часть присваивания");
                //    }
                //    if ((Regex.IsMatch(input[i + 1].Replace(";",""), @"[0-9],[0-9]"))||(Regex.IsMatch(input[i + 1].Replace(";", ""), @"[0-9]")))
                //    {
                //        Console.WriteLine("Неверная правая часть присваивания");
                //    }
                //}
            }
            if (count % 2 != 0)
